 ...
 * @returns {Uint8Array}
 */
export function compress(input, profile) {
    const ptr0 = passArray8ToWasm0(input, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
 ...
 * @param {string} profile
 * @param {any} options
 * @param {Function | null} [on_phase]
 * @returns {any}
 */
export function compress_with_report(input, profile, options, on_phase) {
    const ptr0 = passArray8ToWasm0(input, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(profile, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.compress_with_report(ptr0, len0, ptr1, len1, options, isLikeNone(on_phase) ? 0 : addToExternrefTable0(on_phase));
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('gema_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
